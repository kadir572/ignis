...
#[tauri::command]
pub async fn download_file(app: AppHandle, file_name: &str, thumbnails: Vec<ThumbnailDownloadData>, _password_map: Option<HashMap<String, String>>) -> Result<String, String> { // password_map: file_path -> password
    ...
    for thumb in &thumbnails {
        let doc = loaded_docs
            .entry(thumb.file_path.clone())
            .or_insert_with(|| {
                Document::load(&thumb.file_path)
                    .expect("Failed to load PDF") // Optionally handle this better
            });

        let pages = doc.get_pages();
        let page_id = match pages.get(&((thumb.page_index + 1) as u32)) {
            Some(id) => *id,
            None => return Err(format!("Page {} not found in {}", thumb.page_index, thumb.file_path)),
        };
    ...
    //     let temp_pdf_path = get_temp_pdf_path(&first_thumb.file_path);
    //     if temp_pdf_path.exists() {
    //         // Copy the temporary PDF to the output location
    //         fs::copy(temp_pdf_path, &output_path)
    //             .map_err(|e| format!("Failed to copy PDF: {}", e))?;
    //     } else {
    //         // If no temporary PDF exists, convert the text file
    //         let text = fs::read_to_string(&first_thumb.file_path)
    //             .map_err(|e| format!("Failed to read txt: {}", e))?;
    //         convert_text_to_pdf(&app, &text, &output_path)?;
    //     }
